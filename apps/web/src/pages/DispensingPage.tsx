...
export function DispensingPage() {
  ...
  return <div className="space-y-6">
    ...
    <Card title="Dispense medicine" description="Search for the patient, then choose an available medicine batch. Expired and insufficient batches cannot be dispensed."><form className="grid gap-3 p-5 sm:grid-cols-2 lg:grid-cols-[minmax(220px,1.5fr)_2fr_100px_1fr_auto] lg:items-end" onSubmit={(event) => { event.preventDefault(); if (!form.patientId) return; dispense.mutate(); }}><PatientPicker value={form.patientId} onChange={(patientId) => setForm((current) => ({ ...current, patientId }))} /><label><span className="field-label">Medicine batch</span><select required value={form.medicineBatchId} onChange={(event) => setForm({ ...form, medicineBatchId: event.target.value })} className="field-input"><option value="">Select batch</option>{batches.map((batch) => <option key={batch.id} value={batch.id}>{batch.medicineName} · {batch.batchNumber} · {batch.quantity} {batch.unit}s</option>)}</select></label><label><span className="field-label">Quantity</span><input required type="number" min="1" value={form.quantity} onChange={(event) => setForm({ ...form, quantity: event.target.value })} className="field-input" /></label><label><span className="field-label">Instructions</span><input value={form.instructions} onChange={(event) => setForm({ ...form, instructions: event.target.value })} className="field-input" placeholder="After meals" /></label><Button disabled={!form.patientId || dispense.isPending}><ClipboardPlus className="h-4 w-4" />Dispense</Button></form>{dispense.isError && <p className="px-5 pb-4 text-[12px] text-rose-600">Unable to dispense. Check the patient, batch, quantity, and expiration date.</p>}</Card>
